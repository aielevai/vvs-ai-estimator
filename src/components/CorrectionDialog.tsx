...
export const CorrectionDialog: React.FC<CorrectionDialogProps> = ({
  ...
  const handleSave = async () => {
    if (!reasoning.trim()) {
      toast({
        title: "Mangler begrundelse",
        description: "Skriv venligst hvorfor du lavede denne rettelse",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('save-correction', {
        body: {
          correction_type: changes[0]?.field === 'labor_hours' ? 'hours_adjustment' : 'general',
          correction_value: correctionValue,
          original_value: Object.fromEntries(changes.map(c => [c.field, c.original_value])),
          corrected_value: Object.fromEntries(changes.map(c => [c.field, c.new_value])),
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4">
          ...
          {/* Reasoning */}
          <div className="space-y-2">
            <Label htmlFor="reasoning">Hvorfor ændrede du dette?</Label>
            <Textarea
              id="reasoning"
              placeholder="F.eks.: 'Kælderarbejde tager altid længere pga. vanskelig adgang' eller 'Kundens materialer er af lavere kvalitet'"
              value={reasoning}
